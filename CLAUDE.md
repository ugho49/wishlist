...
- **For Button navigation**: When using `component={Link}`, you may need proper type assertions. **Alternative approach**: Use `onClick={() => navigate({ to: '/' })}` with `useNavigate()` hook
...
#### TanStack Router Guidelines (IMPORTANT)
- **File-based routing**: All routes are defined in `apps/front/src/routes/` directory
- **Route naming conventions**:
  - `__root.tsx` - Root layout with providers
  - `index.tsx` - Index route for a path segment
  - `_layout.tsx` - Pathless layout route (e.g., `_authenticated.tsx`, `_anonymous.tsx`)
  - `$param.tsx` - Dynamic route parameter (e.g., `$eventId.tsx`)
  - `path.nested.tsx` - Nested route (e.g., `events.new.tsx` = `/events/new`)
- **Route tree generation**: Auto-generated by `@tanstack/router-plugin` in `routeTree.gen.ts` (gitignored)
- **Navigation**:
  - Use `useNavigate()` from `@tanstack/react-router`
  - Call with object syntax: `navigate({ to: '/path', params: { id: '123' } })`
  - For type-safe params, import Route from route file: `const { eventId } = Route.useParams()`
- **Route guards**: Implemented in `beforeLoad` function in layout routes (e.g., `_authenticated.tsx` checks authentication)
- **Router context**: Defined in `__root.tsx`, includes `queryClient`, `isAuthenticated`, and `accessToken`
- **Redirects**: Use `redirect()` from `@tanstack/react-router` and `throw` it in `beforeLoad`
- **Links**: Use `<Link>` from `@tanstack/react-router` or the custom `RouterLink` component for MUI integration
