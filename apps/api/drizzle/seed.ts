...
function getRandomUserWithExclusion(exclude: string) {
  ...
  const users = usersValues.filter(candidate => candidate.id !== exclude);
...
async function main() {
  ...
  for (const user of usersValues) {
    ...
    const getRandomUserAttendeeEventIdWithExclusion = (exclude: string[]) => {
      ...
      const eventIds = userAttendeeEventIds.filter(id => !exclude.includes(id));
  ...
  for (const item of itemsValues) {
    const wishlist = wishlistsValues.find(candidate => candidate.id === item.wishlistId);
