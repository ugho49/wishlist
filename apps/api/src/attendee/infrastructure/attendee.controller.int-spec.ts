...
describe('AttendeeController', () => {
      ...
      it.each([
        {
          body: {},
          case: 'empty body',
          message: ['event_id should not be empty', 'email should not be empty'],
        },
        {
          body: { event_id: 'invalid-uuid', email: 'not-an-email' },
          case: 'invalid event_id and email',
          message: ['email must be an email'],
        },
        {
          body: { event_id: uuid(), email: 'test@example.com', role: 'invalid-role' },
          case: 'invalid role',
  ...
  describe('DELETE /attendee/:id', () => {
    const path = (id: string) => `/attendee/${id}`

    it('should return unauthorized if not authenticated', async () => {
      const request = await getRequest()
      const attendeeId = uuid()

      await request.delete(path(attendeeId)).expect(401)
    })
    ...
    describe('when user is authenticated', () => {
      ...
      it('should return 404 when attendee does not exist', async () => {
        ...
        await request
          .delete(path(nonExistentAttendeeId))
          .expect(404)
          .expect(({ body }) =>
            expect(body).toMatchObject({
              error: 'Not Found',
      ...
      it('should return 409 when trying to delete yourself', async () => {
        const { attendeeId } = await fixtures.insertEventWithMaintainer({
          title: 'Test Event',
          description: 'Test Description',
          maintainerId: currentUserId,
        })
      ...
      it('should delete pending attendee successfully', async () => {
        ...
        await request.delete(path(attendeeId)).expect(200)
