import { BadRequestException, ForbiddenException, Inject, UnprocessableEntityException } from '@nestjs/common'
import { CommandHandler, EventBus, IInferredCommandHandler } from '@nestjs/cqrs'
import { AttendeeRepository } from 'apps/api/src/attendee/domain/attendee.repository'
...
export class StartSecretSantaHandler implements IInferredCommandHandler<StartSecretSantaCommand> {
  ...
  async execute(command: StartSecretSantaCommand): Promise<void> {
    let secretSanta = await this.secretSantaRepository.getSecretSantaForUserOrFail({
    ...
    await this.transactionManager.runInTransaction(async tx => {
      await this.secretSantaUserRepository.save(secretSanta.users, tx)
