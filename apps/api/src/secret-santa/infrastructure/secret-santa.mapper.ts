...
function toSecretSantaDto(model: SecretSanta, event: Event, attendees: Attendee[]): SecretSantaDto {
  return {
    id: model.id,
    ...
    users: model.users.map(user => toSecretSantaUserDto(user, attendees.find(a => a.id === user.attendeeId)!)),
