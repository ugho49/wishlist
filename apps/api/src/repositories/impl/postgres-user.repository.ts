...
export class PostgresUserRepository implements UserRepository {
    ...
    await client
      ...
      .onConflictDoUpdate({
        ...
        set: {
          ...
          giftNotes: user.giftNotes ?? null,
          addressLine1: user.address?.line1 ?? null,
          addressLine2: user.address?.line2 ?? null,
  ...
  static toModel(row: typeof schema.user.$inferSelect): User {
    return new User({
      ...
      clothingSize: row.clothingSize ?? undefined,
      shoeSize: row.shoeSize ?? undefined,
      giftNotes: row.giftNotes ?? undefined,
      address:
        row.addressLine1 && row.addressPostalCode && row.addressCity && row.addressCountry
          ? {
